...
class Board:
    ...
    def initialize(self, init_pose):
        ...
        for pair in self.edge_indices:
            edge = vedo.Line(init_pose[pair], lw=2, c='w')
            self.board['edges'].append(edge)
        
        # Add joint actors to joints list
        color = ['w', 'b', 'yellow', 'g']
        for i, coords in enumerate(init_pose):
            vertex = vedo.Sphere(coords, r=0.015, c=color[i%4])
...
class Cameras:
    ...
    def initialize(self):
        ...
        for idx, (rotmat, transl) in enumerate(zip(rotmats, transls)):
            ...
            camera_rotmat = rotmat
            camera_transl = - transl @ rotmat
    ...
    @staticmethod
    def get_name(nametxt, transl, scale=0.050):  # 0.05
        name = vedo.Text3D(nametxt, pos=transl, s=scale)
        return name
    
    @staticmethod
    def get_pyramid(rotmat, transl, scale=0.1): # 0.1
    ...
    @staticmethod
    def get_axes(rotmat, transl, scale=0.20):    # 0.2
        cam_x_axis = np.matmul([1, 0, 0], rotmat.T) # Red
        cam_y_axis = np.matmul([0, 1, 0], rotmat.T) # Green
        cam_z_axis = np.matmul([0, 0, 1], rotmat.T) # Blue
